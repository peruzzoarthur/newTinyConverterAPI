...
async function populateCryptoList(): Promise<string[]> {
  const requestURL = "https://api.exchangerate.host/cryptocurrencies";
  try {
    ...
    const symbols = Object.keys(cryptocurrencies).map(
      (key) => cryptocurrencies[key].symbol
    );
    const names = Object.keys(cryptocurrencies).map(
      (key) => cryptocurrencies[key].name
    );
    return symbols;
  } catch (error) {
    console.error(error);
...
const CryptoList = ({
  selectedCurrency,
  setSelectedCurrency,
}: CryptoListProps): JSX.Element => {
  const [symbols, setSymbols] = useState<string[]>([]);

  useEffect(() => {
    async function fetchCryptoCurrencies() {
      const symbols = await populateCryptoList();
      setSymbols(symbols);
    }
